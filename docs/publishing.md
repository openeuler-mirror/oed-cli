...
| 位置 | 字段 | 谁用它 |
|---|---|---|
| `pyproject.toml:7` | `version = "0.3.0"` | wheel 文件名 + PyPI 项目页版本 |
| `src/oed_cli/__init__.py:5` | `__version__ = "0.3.0"` | `oed --version` 的输出、`http.py` 的 `User-Agent` 头(`oed/0.3.0 (+...)`)|
...
```bash
sed -i 's/version = "0.2.3"/version = "0.3.0"/' pyproject.toml
sed -i 's/__version__ = "0.2.3"/__version__ = "0.3.0"/' src/oed_cli/__init__.py
```

> ⚠️ 改完第二处后,User-Agent 会变成 `oed/0.3.0 ...`。CloudWAF 跟 UA 没有强耦合(就是 `http.py:30` 那条),但**如果你之前因为 WAF 拦了默认 UA 而改过 `OED_USER_AGENT` 环境变量**,那是另一回事,跟这里无关。
...
```
dist/oed_cli-0.3.0-py3-none-any.whl
dist/oed_cli-0.3.0.tar.gz
```

> 🚨 **文件名中间的 `0.3.0` 改成新版本号了么?** 没改就是没生效 —— 见第 7 节"常见问题 2"。
...
pip install --index-url https://test.pypi.org/simple/ \
            ...
            oed-cli==0.3.0
oed --version    # 应该输出: oed, version 0.3.0
```
...
- 直接拉: `pip install --index-url https://pypi.org/simple/ oed-cli==0.3.0`
